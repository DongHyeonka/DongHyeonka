...
**인증/인가 설계부터 운영 인프라까지 한 사이클을 직접 구축**해 본 신입 백엔드 엔지니어입니다.  
Keycloak · Vault · K3s로 인증 플랫폼을 만들고, 쿼리 튜닝으로 응답 시간을 231배 단축한 경험이 있습니다.
...
### Keycloak 기반 인증 플랫폼 — 인증 서버 + 운영 인프라

> 개인 프로젝트 · 2025.11 ~ 현재 · 설계/구현/운영 전담  
> [인증 서버](https://github.com/DongHyeonka/Project-Auth-Server) · [운영 인프라](https://github.com/DongHyeonka/Project-Infra)

인증 구현에서 끝내지 않고, **인증 위임 → Ingress 단 차단 → 시크릿 파이프라인까지 한 사이클을 직접 경험**하기 위해 시작했습니다.
...
- **Ingress 단 인증 차단 구조** — Traefik ForwardAuth + oauth2-proxy + Keycloak 조합으로 미인증 요청이 백엔드에 도달하지 못하게 함. 백엔드 코드의 인증 분기를 인프라로 위임 → 컨트롤러에서 인증 코드 0 줄
...
- **kustomize build / kubeconform / kube-linter 3 단 검증 자동화** — 모든 overlay 에 대해 빌드 / 스키마 / 안티패턴을 단일 스크립트로 검증 (PR 게이트)
